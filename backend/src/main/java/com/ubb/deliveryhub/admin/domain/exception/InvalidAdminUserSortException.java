...
public class InvalidAdminUserSortException extends RuntimeException {
    ...
    public InvalidAdminUserSortException(String invalidProperty, Collection<String> allowedProperties) {
    ...
    private static String buildDetail(String invalidProperty, Collection<String> allowedProperties) {
        return "Invalid sort property: %s. Allowed: %s".formatted(
            invalidProperty,
            String.join(", ", sortedCopy(allowedProperties))
        );
    }
