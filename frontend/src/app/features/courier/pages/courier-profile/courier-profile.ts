...
interface CourierProfileLoadErrorState {
  title: string;
  message: string;
}

@Component({
  selector: 'app-courier-profile-page',
  imports: [
    ReactiveFormsModule,
    Card,
    Button,
  ...
  templateUrl: './courier-profile.html',
  changeDetection: ChangeDetectionStrategy.OnPush,
})
export class CourierProfilePage {
  ...
  protected readonly loadError = signal<CourierProfileLoadErrorState | null>(null);

  private lastLoadedSnapshot: CourierProfileDto | null = null;
  private lastLoadedComparable: CourierProfileUpdateRequest | null = null;
      ...
      .subscribe({
        next: () => {
          ...
          this.messageService.add({
            ...
            detail: 'Your courier profile was saved successfully.',
            life: 4000,
          });
          this.loadProfile();
        },
        error: (error: unknown) => {
          if (
            ...
            this.courierProfileService.applyValidationErrors(this.form, error)
          ) {
            this.form.markAllAsTouched();
            return;
          }
  ...
  private applyProfileToForm(profile: CourierProfileDto): void {
    ...
    this.form.patchValue(
      {
        ...
        availability: {
          availableNow: profile.availability.availableNow,
          expressCapable: profile.availability.expressCapable,
        },
      },
      { emitEvent: false },
    );
  ...
  private buildPayload(): CourierProfileUpdateRequest {
    ...
    return {
      ...
      availability: {
        availableNow: raw.availability.availableNow,
        expressCapable: raw.availability.expressCapable,
      },
    };
  }
  ...
  private toLoadErrorState(error: unknown): CourierProfileLoadErrorState {
    if (error instanceof HttpErrorResponse && error.status === 404) {
      return {
        title: 'Profile not found',
        message:
          'Your courier profile is not initialized yet. Contact an administrator.',
      };
    }
    ...
    if (error instanceof HttpErrorResponse && error.status === 403) {
      return {
        title: 'Access denied',
        message: 'Your account cannot access courier profile settings.',
      };
    }
